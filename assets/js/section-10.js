...
async function walletConnectedPortal() {
  ...
  if (window.touchScreen) {
    ...
    document.querySelector("p#section-10-portal-instructions").textContent =
      "Unfortunately the MetaMask app is not compatible with the portal. Please use a desktop browser instead.";
  }
  ...
  try {
    const darcels = await getData(
      `https://eth-mainnet.alchemyapi.io/v2/kA0GvyDvzb_9brFE0cU4YM5cKdbdmWe9/getNFTs/?owner=${await getWalletAddress()}&contractAddresses[]=0x8d609bd201beaea7dccbfbd9c22851e23da68691`
    );
  ...
  if (nfts.length > 0) {
    ...
    container.innerHTML =
      '<p>No Dour Darcels found :-(<br /><br /><a href="https://opensea.io/collection/dourdarcels" target="_blank">View on OpenSea</a></p>';
  }
  ...
  format.addEventListener("change", async (e) => {
    ...
    if (val === "hi-res") {
      ...
      portalToggleDownload("enabled");
    } else if (val === "animated-gifs") {
      download.href = "https://dourdarcels.s3.amazonaws.com/zip/Animated+GIFs.zip";
      download.setAttribute("download", "Dour Darcels Animated GIFs");
      portalToggleDownload("enabled");
    } else if (val === "emojis") {
      download.href = "https://dourdarcels.s3.amazonaws.com/zip/Emojis.zip";
      download.setAttribute("download", "Dour Darcels Emojis");
      portalToggleDownload("enabled");
    } else if (val === "logo-icon") {
      download.href = "https://dourdarcels.s3.amazonaws.com/zip/Logo+%2B+Icon.zip";
      download.setAttribute("download", "Dour Darcels Logo + Icon");
      portalToggleDownload("enabled");
    } else if (val !== "") {
      portalGenerate(val);
    } else {
...
async function portalGenerate(format) {
  ...
  try {
    ...
    if (window.generateDate === date) {
      ...
      bg.src = `https://dourdarcels.s3.amazonaws.com/bg/${window.portalBg
        .toLowerCase()
        .replace(/ /g, "_")}.png?date=${date}`;
